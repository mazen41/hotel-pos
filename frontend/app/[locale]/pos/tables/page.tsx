...
export default function TablesPage() {
  ...
  useEffect(() => { if (can('tables.view') || can('tables.edit')) loadTables(); }, [can, loadTables]);
  ...
  if (!can('tables.view') && !can('tables.edit')) return <div className="flex min-h-screen items-center justify-center text-text-muted">You do not have permission to manage tables.</div>;

  return (
    <div dir={isRtl ? 'rtl' : 'ltr'} className="min-h-screen bg-background p-3 sm:p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h1 className="font-display text-2xl font-bold text-text-primary sm:text-3xl">Tables</h1><p className="text-text-muted">Manage dining room tables and availability.</p></div><div className="flex gap-2">{can('tables.create') && <button onClick={() => setForm(emptyForm)} className="flex items-center gap-2 rounded-xl bg-primary px-5 py-3 font-semibold text-white shadow-medium hover:bg-primary/90"><Plus className="h-4 w-4" />Add Table</button>}<button onClick={loadTables} className="rounded-xl border border-border px-4 py-3 text-text-secondary hover:bg-surface-hover"><RefreshCw className="h-4 w-4" /></button></div></div>
